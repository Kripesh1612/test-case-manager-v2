...
| 6 | An admin in project A reads or mutates rows in project B | I, T, E | Every data-access path goes through `projectScope(req.user)` so Prisma `where` always carries `project_id: req.user.projectId`; a query for a foreign `projectId` returns 404, never 200, leaking no existence signal. Coverage extends across the multi-tenant surfaces (projects, webhooks, digest, visual, scheduled jobs) — each new tenant-scoped table was wired through `projectScope` from day one rather than retrofitted. | `utils/scope.js`, applied across `routes/*.js` | audit-batch-E + multi-tenant hardening |
...
| 10 | A bulk-list query returns an unbounded response, OOM-ing the server or the React tree | D | Every list endpoint is paginated. The audit endpoint uses `clampInt(req.query.limit, 1, 500, 50)` — a deliberately low default for a moderation surface. Other list endpoints use their own per-domain defaults (e.g. `100` for runs, `200` for cases). All endpoints hardened with explicit integer parsing — `?actor_id=abc` returns 400, not silently filters to actor 1. | `utils/params.js`, `routes/*.js`, `routes/audit.js` | audit-batch-C, audit-batch-D |
