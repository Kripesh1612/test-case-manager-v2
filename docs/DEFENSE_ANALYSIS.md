...
| Tests | 292 Cypress tests (133 API + 144 UI + 15 advanced-patterns) across 24 spec files; plus 80 Node `node:test` unit cases (372 total) |
...
7. 292-test Cypress suite covering contract + UI + advanced patterns (plus 80 unit = 372 total).
...
- **24 spec files**, **214 actual tests** (the README badge declares 215 — the badge is slightly stale; one phase added a test without a badge bump).
- Split: 125 API + 83 UI + 6 advanced-patterns.
...
|---|---|
| `production-hardening.md` reads like a real audit. | No `CHANGELOG.md`, `CONTRIBUTING.md`, `SECURITY.md`, ADR. |
| 1:1 module/doc mapping. | No live CI badge (only static "215 passing locally"). |
| Six worked bugs in `bugs-caught-by-tests.md`. | No OpenAPI generated from Zod schemas (acknowledged gap). |
...
4. **[RESOLVED] Test count badge slightly stale** — the static "N passing locally" badge has been replaced with a **live GitHub Actions CI badge**, and hardcoded test-count prose was reconciled to the actual current total (292 Cypress + 80 unit = 372). See `docs/ci.md`.
